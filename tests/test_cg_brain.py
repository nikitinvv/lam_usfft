...
rec_folder = f'/data/2023-04/Nikitin_rec/rec_cg_bin{bin}x{bin}'
data_folder = f'/data/2023-04/Nikitin_rec/data_bin{bin}x{bin}'
...
dxchange.write_tiff_stack(data.real, f'{data_folder}/data.tiff', overwrite=True)
with FFTCL(n0, n1, n2, detw, deth, ntheta, n1c, dethc, nthetac) as slv:    
    u = np.zeros([n1,n0,n2],dtype='complex64')#reshaped
    u = slv.cg_lam(data, u, theta, phi, niter,gamma ,dbg=True,dbg_step=8)        
    u = u.swapaxes(0,1)
