...
n = data.shape[-1]
# ne = (4096+512+128)//2**bin
ne = 3*n//2#4096//2**bin
data = np.pad(data,((0,0),(0,0),(ne//2-n//2,ne//2-n//2)),'edge')
print('pad data')
n0 = data.shape[1]
n1 = data.shape[2]
n2 = data.shape[2]
...
with FFTCL(n0, n1, n2, detw, deth, ntheta, n1c, dethc, nthetac) as slv:    
    ...
    # u = slv.cg_lam(data, u, theta, phi, 64, gamma=1,dbg=True,dbg_step=4)
    # u = u.swapaxes(0,1)
    ...
    u = slv.admm(data, psi, lamd, u, theta, phi, alpha, liter, niter, dbg=False,dbg_step=4,rec_folder=rec_folder)
    u = u.swapaxes(0,1)
    u = u[:,ne//2-n//2:ne//2+n//2,ne//2-n//2:ne//2+n//2]
